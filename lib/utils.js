...
function doBuild(file, outDir = 'build') {
  ...
  try {
    ...
    console.log(`📊 Stats: ${stats.lines} lines of code, ${stats.characters} chars`);
 ...
 * Watch function
 */
function watchAndBuild(file, outDir) {
  const inputFile = path.isAbsolute(file) 
    ? file 
    : path.join(process.cwd(), file);
