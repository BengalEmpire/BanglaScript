function tokenizePreserve(code) {
  ...
  const n = code.length;
  let i = 0;
  let isExpressionStart = true; // Tracks if the next token can start an expression (for regex detection)

  const expressionStarters = new Set([
    // English keywords
    'return', 'throw', 'await', 'delete', 'void', 'typeof', 'new', 'yield',
    // Bangla equivalents
    'ফেরত', 'প্রেরণ', 'ফেলা', 'অপেক্ষা', 'মুছো', 'শূন্য', 'টাইপ', 'নতুন'
  ]);
  ...
  while (i < n) {
    ...
    if (ch === '/' && i + 1 < n && code[i + 1] !== '/' && code[i + 1] !== '*' && isExpressionStart) {
